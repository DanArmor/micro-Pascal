...
std::unique_ptr<IAST> SyntaxAnalyzer::syntaxFuncDef(void) {
    ...
    while (getCurTok().getType() != IToken::RPAREN) {
        ...
        if (getCurTok().getType() == IToken::COMMA) eat(IToken::COMMA);
    }
    eat(IToken::RPAREN);
...
std::unique_ptr<IAST> SyntaxAnalyzer::syntaxBlock(void) {
    ...
    return ASTFactory::createAST({"", IToken::BLOCK, IToken::UNKNOWN},
...
std::vector<std::unique_ptr<IAST>> SyntaxAnalyzer::syntaxVarDecl(void) {
    ...
    for (auto &var : varsList) {
        varDeclsList.emplace_back(ASTFactory::createAST(
            {"", IToken::EMPTY, IToken::VARDECL}, std::move(var),
    ...
    switch (getCurTok().getType()) {
        ...
        case IToken::ID: {
            if (lookFoward().getType() == IToken::ASSIGN ||
                lookFoward().getType() == IToken::LSQBRACKET)
...
std::unique_ptr<IAST> SyntaxAnalyzer::syntaxExpr(void) {
    ...
    if (isIn(getCurTok().getType(),
             {IToken::LESS, IToken::LESS_EQ, IToken::NEQ, IToken::EQ,
              IToken::MORE, IToken::MORE_EQ})) {
        token = getCurTok();
        eat(token.getType());
...
std::unique_ptr<IAST> SyntaxAnalyzer::syntaxSimpleExpr(void) {
    ...
    while (isIn(getCurTok().getType(),
                {IToken::PLUS, IToken::MINUS, IToken::OR})) {
        Token token = getCurTok();
        eat(token.getType());
...
std::unique_ptr<IAST> SyntaxAnalyzer::syntaxTerm(void) {
    ...
    while (isIn(getCurTok().getType(),
                {IToken::FLOAT_DIV, IToken::MUL, IToken::INTEGER_DIV,
                 IToken::MOD, IToken::AND})) {
        Token token = getCurTok();
        eat(token.getType());
        factorPTR =
...
std::unique_ptr<IAST> SyntaxAnalyzer::syntaxIter(void) {
    ...
    if (finTok.getAdvType() != IToken::SOME_CONST and
        finTok.getType() != IToken::ID)
...
void SyntaxAnalyzer::getNextToken(void) { currentIndex++; }

Token &SyntaxAnalyzer::getCurTok(void) { return tokens[currentIndex]; }
...
List<Token> SyntaxAnalyzer::getTokens(void) { return tokens; }